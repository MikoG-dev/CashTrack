...
export default function Income() {
  ...
  const fetchIncome = async (page = 1, limit = rowsPerPage) => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/get/income?${params.toString()}`,
  ...
  const handleEdit = (income) => {
    navigate(`/edit-transaction/${income.id}`, {
      state: { transaction: income, page: "income" },
    });
  };
      ...
      <div className="flex-grow-1">
        <div className="p-4">
          ...
          <div className="d-flex justify-content-between align-items-center mb-4">
            <h2 className="mb-0">Income Transactions</h2>
            <button
              className="btn btn-success"
              onClick={() =>
                navigate("/add-transaction", { state: { page: "income" } })
              }
            >
              + Add
            </button>
